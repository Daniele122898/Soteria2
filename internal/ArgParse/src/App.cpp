            ...
            for (int i = 1; i < argc; ++i) {
                ...
                auto addConsume = [&i, &argv, argc] (const std::shared_ptr<IOption>& op) {
                    if (i+1 >= argc || argv[i+1][0] == '-') {
                        throw std::invalid_argument(fmt::format("Flag '{}' consumes a value but there is none to consume", op->GetName()));
                    }
                ...
                if (arg[1] != '-') {
                    ...
                    while (arg[j] != '\0') {
                        if(auto f = m_allOptions.find(std::string_view{&arg[j], 1}); f != m_allOptions.end()) {
                            ...
                            if (arg[j+1] != '\0') {
                                throw std::invalid_argument(fmt::format("Flag '{}' consumes a value but is not the last option", arg[j]));
                            }
                            addConsume(op);
                            break;
